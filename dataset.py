...
class UncertainTrainTripleDataset(TensorDataset):
    def __init__(self, data_dir, filename, REL_VOCAB_SIZE):
        df = pd.read_csv(join(data_dir, filename), sep='\t', names=['h', 'r', 't', 'p'])

        data1 = df[['h', 'r', 't']].values
        data2 = df[['t', 'r', 'h']].values
        data2[:,1] = data2[:,1] + REL_VOCAB_SIZE
        data = np.concatenate((data1, data2))
        prob = np.concatenate((df['p'].values,df['p'].values))
