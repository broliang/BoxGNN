...
class Box:
    ...
    def __getitem__(self, item):
        return Box(self.min_embed[item], self.max_embed[item])
...
class Graphlayer(nn.Module):
    def __init__(self, params):
        ...
        self.W_rel_trans = self.get_param([params.REL_VOCAB_SIZE, params.DIM])
        self.W_rel_scale = self.get_param([params.REL_VOCAB_SIZE, params.DIM])
    ...
    def comp(self, inter_min, inter_max):
        if self.reduce == 'min':
            min_ = inter_min.max(dim=1)[0]
            max_ = inter_max.min(dim=1)[0]
        elif self.reduce == 'max':
            min_ = inter_min.min(dim=1)[0]
            max_ = inter_max.max(dim=1)[0]
        elif self.reduce == 'mean':
            min_ = inter_min.mean(dim=1)
            max_ = inter_max.mean(dim=1)
        else:
            min_ = None
            max_ = None
        return min_, max_
    ...
    def message_func(self, edges:dgl.udf.EdgeBatch):
        ...
        src_min_embed = edges.src['min']
        src_delta_embed = edges.src['delta']
        relu = nn.ReLU()
        dst_min_embed = edges.dst['min']
        dst_delta_embed = edges.dst['delta']
        dst_max_embed = edges.dst['max']
        dst_box = Box(dst_min_embed, dst_max_embed)
        edge_trans = self.rel_trans[edge_type]
        edge_scale = self.rel_scale[edge_type]
        new_min_embed = src_min_embed + edge_trans
        new_delta_embed = relu(src_delta_embed * edge_scale)
        new_max_embed = new_min_embed + new_delta_embed
        src_box = Box(new_min_embed, new_max_embed)
        intersection = self.intersection(src_box, dst_box)
        log_inter = self.log_volumes(intersection)
        score = self.log_volumes(src_box) - log_inter
        return {'score': score}


    def forward(self, g, x, rel_trans, rel_sacle):
        g = g.local_var()
        g.ndata['min'] = x.min_embed
        g.ndata['delta'] = x.delta_embed
        g.ndata['max'] = x.max_embed
        self.rel_trans = rel_trans
        self.rel_scale = rel_sacle
        g.apply_edges(self.message_func)
        return g.edata['score']

class GraphAFFlayer(nn.Module):
    def __init__(self, params):
        super(GraphAFFlayer, self).__init__()
        self.euler_gamma = 0.57721566490153286060
        self.gumbel_beta = params.GUMBEL_BETA
        self.W_rel_trans = self.get_param([params.REL_VOCAB_SIZE, params.DIM], 0, 1e-4)
        self.W_rel_scale = self.get_param([params.REL_VOCAB_SIZE, params.DIM], 1, 0.2)
        self.reduce = params.reduce
        self.dropout = torch.nn.Dropout(0.2)
    def get_param(self, shape, mean, std):
        param = nn.Parameter(torch.Tensor(*shape))
        torch.nn.init.normal_(param, mean=mean, std=std)  # 1e-4 before
        return param
    ...
    def message_func(self, edges:dgl.udf.EdgeBatch):
        ...
        src_delta_embed = edges.src['delta']
        relu = nn.ReLU()
        edge_trans = self.W_rel_trans[edge_type]
        edge_scale = self.W_rel_scale[edge_type]
        new_min_embed = src_min_embed + edge_trans
        new_delta_embed = relu(src_delta_embed * edge_scale)
        new_max_embed = new_min_embed + new_delta_embed
        return {'min':new_min_embed, 'max': new_max_embed}

    def reduce_func(self, nodes:dgl.udf.NodeBatch):
        new_min_embed = nodes.mailbox['min']
        new_max_embed = nodes.mailbox['max']
        shape = new_min_embed.shape
        now_min_embed = nodes.data['min'].unsqueeze(1).repeat_interleave(shape[1], dim=1)
        now_max_embed = nodes.data['max'].unsqueeze(1).repeat_interleave(shape[1], dim=1)

        new_min_embed = new_min_embed.reshape(shape[0] * shape[1], shape[2])
        new_max_embed = new_max_embed.reshape(shape[0] * shape[1], shape[2])
        new_box = Box(new_min_embed, new_max_embed)

        now_min_embed = now_min_embed.reshape(shape[0] * shape[1], shape[2])
        now_max_embed = now_max_embed.reshape(shape[0] * shape[1], shape[2])
        now_box = Box(now_min_embed, now_max_embed)
        inter_box = self.intersection(new_box, now_box)
        inter_min = inter_box.min_embed.reshape(shape[0], shape[1], shape[2])
        inter_max = inter_box.max_embed.reshape(shape[0], shape[1], shape[2])
        inter_min, inter_max = self.comp(inter_min, inter_max)
        # inter_min, inter_max = self.comp(inter_min, inter_max)
        # return {'min': inter_min.mean(dim = 1), 'max': inter_max.mean(dim = 1)}
        return {'min': inter_min, 'max': inter_max}
    ...
    def forward(self, g, x):
        ...
        min_embed = g.ndata.pop('min')
        max_embed = g.ndata.pop('max')
        x = Box(min_embed, max_embed)
        return x
...
class GumbelBox(nn.Module):
    def __init__(self, device, vocab_size, embed_dim, ratio, min_init_value, delta_init_value, params):
        ...
        min_embedding_2 = self.init_embedding(vocab_size, embed_dim, min_init_value)
        delta_embedding_2 = self.init_embedding(vocab_size, embed_dim, delta_init_value)
        self.min_embedding_2 = nn.Parameter(min_embedding_2)
        self.delta_embedding_2 = nn.Parameter(delta_embedding_2)
        ...
        # make nn.Parameter
        self.rel_trans_for_head, self.rel_scale_for_head = nn.Parameter(
            rel_trans_for_head.to(device)), nn.Parameter(
            rel_scale_for_head.to(device))
        self.rel_trans_for_tail, self.rel_scale_for_tail = nn.Parameter(
            rel_trans_for_tail.to(device)), nn.Parameter(
            rel_scale_for_tail.to(device))


        self.conv1 = GraphAFFlayer(params)
    ...
    def forward_base(self, g):
        x = self.get_entity_boxes(range(self.vocab_size))
        x = self.conv1(g, x)
        return x
    def forward(self, g, ids, probs, train=True):
        head, rel, tail = ids[:, 0], ids[:, 1], ids[:, 2]
        all_entity = self.forward_base(g)
        head_boxes = all_entity[head]
        tail_boxes = all_entity[tail]

        # head_boxes_2 = self.get_entity_boxes_2(head)
        # tail_boxes_2 = self.get_entity_boxes_2(tail)
        # head_boxes = self.concat(head_boxes, head_boxes_2)
        # tail_boxes = self.concat(tail_boxes, tail_boxes_2)
        head_boxes = self.transform_head_boxes(head_boxes, rel)
    ...
    def transform_head_boxes(self, head_boxes, rel):
    ...
    def transform_tail_boxes(self, tail_boxes):
    ...
    def get_entity_boxes_2(self, entities):
        min_rep = self.min_embedding_2[entities]  # batchsize * embedding_size
        delta_rep = self.delta_embedding_2[entities]
        max_rep = min_rep + torch.exp(delta_rep)
        boxes = Box(min_rep, max_rep)
        return boxes
    ...
    def get_entity_boxes_detached(self, entities):
        ...
        # min_rep_2 = self.min_embedding_2[entities].detach()
        # delta_rep_2 = self.delta_embedding_2[entities].detach()
        # min_rep = (min_rep + min_rep_2) / 2
        # delta_rep = (delta_rep + delta_rep_2) / 2
        # min_rep = torch.concat([min_rep, min_rep_2], dim= 1)
        # delta_rep = torch.concat([delta_rep, delta_rep_2], dim= 1)
        max_rep = min_rep + torch.exp(delta_rep)
        boxes = Box(min_rep, max_rep)
        return boxes

    def transitive_rule_loss(self, ids):
        subsets = [ids[(ids[:, 1] == r).nonzero().squeeze(1), :] for r in
                   self.params.RULE_CONFIGS['transitive']['relations']]
...
class BoxGraph(nn.Module):
    def __init__(self, device, vocab_size, embed_dim, ratio, min_init_value, delta_init_value, params):
        super(BoxGraph, self).__init__()
        # super(BiGumbelBox, self).__init__(device, vocab_size, embed_dim, ratio, min_init_value, delta_init_value, params)
    ...
    def forward_base(self, g):
        x = self.get_entity_boxes(range(self.vocab_size))
        score = self.conv1(g, x, self.rel_trans_for_head, self.rel_scale_for_head)
        return score
    def forward(self, g, ids, probs, train=True):

        if train == True:
            return self.forward_base(g), g.edata['probs']

        head, rel, tail = ids[:, 0], ids[:, 1], ids[:, 2]
        head_boxes = self.get_entity_boxes(head)
        tail_boxes = self.get_entity_boxes(tail)
        head_boxes = self.transform_head_boxes(head_boxes, rel)
    ...
    def random_negative_sampling0(self, positive_samples, pos_probs, neg_per_pos=None):
        ...
        corrupted_heads[bad_heads_idxs] = torch.randint(self.vocab_size, (torch.sum(bad_heads_idxs),)).to(
            self.device)
        corrupted_tails[bad_tails_idxs] = torch.randint(self.vocab_size, (torch.sum(bad_tails_idxs),)).to(
            self.device)
