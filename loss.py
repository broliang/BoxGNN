...
def main_mse_loss(model, g, ids, cls):
    criterion = nn.MSELoss(reduction='mean')
    criterionmae = nn.L1Loss(reduction='mean')
    prediction, truth = model(g, ids, cls, train=True)
    mse = criterion(torch.exp(prediction), truth)
    mae = criterionmae(torch.exp(prediction), truth)
    mse +=  0.1 * mae
    return mse
...
def my_loss(model, g, ids, cls, params):
    NEG_RATIO = 1
    pos_loss = main_mse_loss(model, g, ids, cls)

    negative_samples, neg_probs = model.random_negative_sampling(ids, cls)
    neg_loss = main_mse_loss(model, g, negative_samples, neg_probs)
