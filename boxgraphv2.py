...
class RelGumbelBox(nn.Module):
    def __init__(self, device, vocab_size, embed_dim, ratio, min_init_value, delta_init_value, params):
        super(RelGumbelBox, self).__init__()
        # super(BiGumbelBox, self).__init__(device, vocab_size, embed_dim, ratio, min_init_value, delta_init_value, params)

        self.conv1 = RelGraphConv(params.DIM, params.DIM, params.REL_VOCAB_SIZE)
        self.conv2 = RelGraphConv(params.DIM, params.DIM, params.REL_VOCAB_SIZE)
    ...
    def forward_base(self, g):
        ...
        x1 = x.min_embed
        x2 = x.max_embed
        x1 = self.conv1(g, x1, g.edata['type'])
        x2 = self.conv2(g, x2, g.edata['type'])
        return x1, x2
    def forward(self, g, ids, probs, train=True):
        head, rel, tail = ids[:, 0], ids[:, 1], ids[:, 2]
        if self.gnn is True:
            min_embedding, max_embedding = self.forward_base(g)
            head_boxes = Box(min_embedding[head], max_embedding[head])
            tail_boxes = Box(min_embedding[tail], max_embedding[tail])
        else:
        ...
        pos_predictions = nn.functional.sigmoid(log_prob)
