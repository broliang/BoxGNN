...
def evaluate_ndcg(g, model, hr_map, num_entity):
    tester = Tester(model, num_entity)
    mean_linear_ndcg, mean_exp_ndcg = tester.mean_ndcg(g, hr_map)
    return mean_linear_ndcg, mean_exp_ndcg


def test_func(g, test_data, device, model, params, threshold=None, neg_mse_also=False, ndcg_also=False):
    ...
    for ids, cls in data:
        ...
        with torch.no_grad():
            prediction, truth = model(g, ids, cls)
            ...
            neg_prediction, _ = model(g, negative_samples, neg_probs)
...
def train_func(g, train_data, train_test_data, dev_data, test_data,
    ...
    for ids, cls in data:
        ...
        loss, pos_loss, neg_loss, logic_loss = my_loss(model, g, ids, cls, params)
        ...
        if step % 20 == 0 or (batch_size > 4096 and step % 10 ==0):
            ...
            # test
            test_MSE, test_MAE, _, _ = test_func(g, test_data, device, model, params, ndcg_also=False)
            wandb.log({'Test MSE': test_MSE})
            wandb.log({'Test MAE': test_MAE})

            # validation
            valid_pos_mse, valid_mae, valid_neg_mse, _ = test_func(g, dev_data, device, model, params, neg_mse_also=True)
...
def run_train(g,
    ...
    for epoch in range(params.EPOCH):
        ...
        loss, best_metric = train_func(g, train_dataset, train_test_dataset, dev_dataset, test_dataset,
        ...
        if params.early_stop == 'ndcg' and epoch % 10 == 0:
            print('####NDCG####')
            linear_ndcg, exp_ndcg = evaluate_ndcg(g, model, params.hr_map, params.VOCAB_SIZE)
...
class Tester:
    ...
    def get_t_ranks(self, g, h, r, ts):
        ...
        with torch.no_grad():
            for ids in candidates_data:  # only one batch
                ...
                log_scores, _ = self.model(g, ids, cls)
                scores = log_scores
    ...
    def ndcg0(self, g, h, r, tw_truth):
        ...
        ranks = self.get_t_ranks(g, h, r, ts)
    ...
    def mean_ndcg(self, g, hr_map):
        ...
        for h in hr_map:
            for r in hr_map[h]:
                ...
                ndcg, exp_ndcg, ranks = self.ndcg(g, h, r, tw_truth)  # nDCG with linear gain and exponential gain
